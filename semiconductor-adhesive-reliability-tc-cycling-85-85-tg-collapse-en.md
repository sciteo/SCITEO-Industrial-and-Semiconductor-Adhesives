...
| Item | Value |
|---|---|
| Shear Strength after TC 1000 | ≥28 MPa |
| Shear Retention after 85/85 | >24 MPa |
| Tg Retention after 85/85 | >140 °C |
| CTE (Below Tg) | <25-30 ppm/°C |
| Volume Resistivity after 85/85 | 10¹³ Ω·cm |
| High-Temp System Tg | >200 °C |
...
TC testing (JESD22-A104F.01) does not damage a package through cold or heat alone. What does the work is the strain-rate differential created by violent temperature swings, plus the viscoelastic strain that accumulates during dwell. Silicon, organic substrates, metal leadframes, and ceramic bases differ widely in CTE, and the adhesive layer trapped between them absorbs tear-level micro-shear. Damage per cycle is small; only after a few hundred cycles do creep and fatigue add up into a macroscopic crack.
...
In EV control units and 800 V traction inverters, SiC MOSFET junction temperatures routinely exceed 175 °C, and AQG 324-qualified automotive modules now list 200 °C junction temperature as a short-duration capability. Layering power cycling on top of TC puts the packaging material under two questions at once: whether the interface can hold a thermal path through hundreds of excursions, and whether the material itself keeps losing mass or turning to powder at temperature.
...
Traditional silicones often exceed 200 ppm/°C in CTE, producing substantial volumetric shift during thermal cycling; standard epoxies fail at the opposite extreme, turning brittle and losing elasticity in the cold. SCITEO's reproduction validation on 800G/1.6T modules showed that neither material buffered the stress between quartz glass and the metal housing. The optical path drifted by micrometers and the signal dropped out. For a link carrying safety redundancy, that failure mode has no acceptable probability. As co-packaged optics (CPO) moves the optical engine inside the package, a 200G-per-lane CPO Ethernet switch built around a 102.4 Tb/s switch ASIC has entered volume production, and interface materials must now withstand higher local heat flux and stricter low-outgassing limits at the same time.
...
Sealing layers in military sensors and deep-sea probes must hold stable electrical insulation through salt fog or sustained moisture. Atmospheric 85/85 (with a national-standard counterpart in GB/T 2423.3 / IEC 60068-2-78 steady-state damp heat), pressurized HAST, and surface insulation resistance (SIR) monitoring are normally run in parallel.
...
The consequences cascade. When the device runs at full load in summer heat with internal temperatures at 120 °C, or passes through reflow soldering later, the actual temperature has already crossed the depressed Tg. CTE then runs from 30 ppm/°C to over 100 ppm/°C, and residual moisture flashes into vapor. Add the popcorn effect and a die can crack from the inside within seconds. Damp-heat reliability is therefore read from the saturated Tg (measured by DMA, logging the tanδ peak position and storage modulus alongside it), not from the Tg printed at shipment; equilibrium water absorption and the coefficient of hygroscopic expansion (CHE) set how far that Tg moves, and reflow preconditioning follows the moisture sensitivity level of IPC/JEDEC J-STD-020.
...
Countering shear in TC is not a matter of harder or softer. SCITEO's approach is to control polymer phase reconfiguration: below Tg the material holds an ultra-low CTE (< 25 ppm/°C) so it deforms in step with silicon wafers and ceramic substrates, keeping micro-stress stable. The modulus system simultaneously keeps a toughness margin in reserve. Between -40 °C and -60 °C the material is not permitted to embrittle; it must dissipate stress through micro-slip of polymer chains and protect fragile structures such as micro-bumps and TSVs.
...
Against insulation failure and ion migration under double-85, the priority is to cut off the carriers at their source. SCITEO electronic-grade packaging materials use stringent purification during synthesis, holding halogens and free alkali-metal ions at very low levels (Cl⁻/K⁺ < 0.8 ppm, quantified by ion chromatography). A highly crosslinked molecular network then compresses the physical permeation path for water, keeping absorption below 0.03%. Cure shrinkage is an equally early indicator: held below 0.2% per ISO 2577, it prevents the interface from being pulled open during crosslinking, which would otherwise leave a moisture path in place before testing even begins. Even after 1,000 hours of 85 °C/85% RH bias, the material still holds a high-impedance state of 10¹³ Ω·cm.
...
The industry has already pushed that line to 230 °C: solid epoxy encapsulation aimed at next-generation SiC power modules has reached a 230 °C Tg and entered mass production. The hard part is lifting Tg without lifting modulus. Higher crosslink density raises interfacial stress, which makes delamination and cracking more likely, so high Tg and low stress have long been a trade-off.
...
The boundary of a reliability test is rarely drawn by the standard itself. It gets pushed around by package architecture and service environment. Several shifts now underway have already made the classic TC and double-85 envelopes feel insufficient.
...
Superconducting qubits operate in the 10-20 mK dilution-refrigerator range, while chip-scale photonic approaches run at around 1.55 K on closed-cycle cooling, and a prototype integrating tin-vacancy color centers with photonic integrated circuits on a single chip has already been demonstrated. Interfacial materials in the liquid-helium domain must answer three questions at once: extreme low-temperature embrittlement, multi-material CTE mismatch, and ultra-low outgassing, assessed to the ASTM E595 total mass loss and CVCM criteria. Conventional organic polymer adhesives tend to crack, outgas, or lose optical alignment under deep cryogenic, high-radiation, and ultra-high-vacuum conditions, which is why inorganic bonding routes such as hydroxide catalysis bonding (HCB) are re-entering packaging process discussions. Their stress logic is a mirror image of the double-85 damp-heat logic: one regime is dominated by moisture uptake and ion migration, the other by thermal contraction differentials and cryogenic brittleness, while both demand identical purity and interfacial integrity.
...
High humidity drives water molecules into the polymer through its free-volume network, while elevated temperature accelerates hydrolysis. If the formulation retains free halide or alkali metal ions, water becomes their transport medium, and electrical bias drives directional ion migration, electrochemical migration (ECM) and dendritic metal growth. Volume resistivity drops by several orders of magnitude within a few hundred hours, surface insulation resistance (SIR) degrades in step, and the end state is leakage-current spikes, dielectric breakdown and insulation failure. The control levers are reducing mobile ion content during synthesis, quantified by ion chromatography (IC), and compressing the moisture permeation path with a densely crosslinked network.
...
- DMA Dynamic Mechanical Analysis (Tg, tanδ peak position and storage modulus)
